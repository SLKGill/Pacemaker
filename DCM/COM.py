...
class COM:
    #Pacemaker serial communication class
    ...
    def serialList(self):
        # Gets list of connected COM ports

        return comports()

    def serOpen(self, serPort):
        # Opens specified port for serial communication
    ...
    def serClose(self):
        # Closes open port to stop serial communication

        self.ser.close()
        self.conn = False
    
    def serState(self):
        # Checks if current port is still open for communication
    ...
    def serWrite(self, paramList):
        # Writes specified parameters to the pacemaker board
        ...
        reactTime = paramList[14] * 100
        resFactor = paramList[15]
        recTime = paramList[16] * 300
        AVdelay = paramList[17]

        # Packing the parameters to into the appropriate amount of bytes based on the data type
        # A detailed list of the parameters and their types can be found in the part2_group5 doc
        ...
        # Function code "55" writes data to the pacemaker board
        ...
        self.ser.write(data) # Writes the data to the pacemaker board
        sleep(0.25)

    def serRead(self):
        # Reads and returns a list of the parameters the board is currently set to

        self.ser.write(b"\x16\x22" + b"\x00"*50) # Function code "22" reads data from the pacemaker board
        sleep(0.25)
        # Note: 50 bytes are being read because that is the number of bytes the board expects to receive
        data_r = self.ser.read(50)
        
        # Unpacking the bytes from the pacemaker board into their respective data types
        # A detailed list of the parameters and their types can be found in the part2_group5 doc
    ...
    def startEgram(self):
        # Reads data from the electrocardiogram pins

        self.ser.write(b"\x16\x47" + b"\x00"*50) # Function code "47" begins transmission of electrocardiogram data
        sleep(0.25)
            
        data_r = self.ser.read(50)

        # Note: Signal values are multiplied by 3.3 to make variations more noticeable
        aSignal = struct.unpack("d", data_r[0:8])[0] * 3.3 
        vSignal = struct.unpack("d", data_r[8:16])[0] * 3.3
        return aSignal, vSignal # Returns data as a list of numbers

    def stopEgram(self):
        # Stop transmission of electrocardiogram data

        self.ser.write(b"\x16\x62" + b"\x00"*50) # Function code "62" ends transmission of electrocardiogram data
        sleep(0.25)
